...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        forkResult = fork();
        if (forkResult == 0) {
            printf("\nI am child with PID [%ld] Obeying your Command\n\n", (long)getpid());
        ...
        if (forkResult > 0) {
            ...
            printf("\tChild's CPU time: %ld.%06d seconds\n", usage.ru_utime.tv_sec, usage.ru_utime.tv_usec);

            printf("\tNumber of involuntary context switches experienced by the child process: %ld\n", usage.ru_nivcsw);
        }
